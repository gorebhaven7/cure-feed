...
def GETbyIngredient(ingredient):
    db = get_database()
    recipes_reduced = db["recipes"]  # specify collection name
    # query using ingredient parameter, exclude id field for cleaner output
    item_details = recipes_reduced.find({"Ingredients": ingredients}, {"_id": 0})
...
def GETbyName(name):
    db = get_database()
    recipes_reduced = db["recipes"]  # go to recipe_info collection
